...
def load_runbook_notes() -> dict[str, str]:
    """Authored operator notes, colocated in runbooks/<feature>.md (optional).

    This is the *authored* half of a runbook (idea 04). The derived half (how to
    toggle, what it gates) is assembled from the graph; these notes carry the human
    judgment a parser can't derive. Authored prose → would be Tier-2 freshness-gated.
    """
    d = REPO / "runbooks"
    notes: dict[str, str] = {}
    if d.exists():
...
def feature_of(flag_node: dict, gated: list[dict]) -> str:
    """The feature name a flag implements (taken from the functions it gates)."""
    for g in gated:
...
def render(nodes: list[dict], edges: list[dict], sha: str, stamp: str) -> dict:
    ...
    notes = load_runbook_notes()
    ent_by_id = {n["id"]: n for n in entities}

    # Runbook-<feature>.md — hybrid (idea 04): derived toggle + affected functions,
    # assembled from the graph, plus authored operator notes from runbooks/<feature>.md.
