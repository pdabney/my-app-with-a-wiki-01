...
def resolve(store: LinkStore, code: str) -> str:
    ...
    @intent Resolving is a pure lookup; it does not record a click (that is the
        analytics module's job) so that read-only previews don't inflate counts.
        An unknown code is an error, not an empty result, so callers can't redirect
        to nowhere.
