...
name: infra
description: >
  Contract-first IaC workflow. Capture required tags and intent before writing any
  CloudFormation or Terraform resource. Use when adding infrastructure, tagging
  resources, or when the user runs /infra.
metadata:
  short-description: "Contract-first IaC tags + intent"
---
...
6. **Verify** with the required-tags gate:

   ```bash
   python3 /path/to/dist-brain-metadata-tooling/engine/check_tags.py --root .
   ```

   A taggable resource missing any required key fails the build.

## Brain context

If `dist-brain` MCP is enabled, use `dist_brain__search` for existing infra entities
and `dist_brain__neighbors` for impact before adding resources.
